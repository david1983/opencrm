...
const standardNavigation = [
  ...
  { name: 'Leads', href: '/leads', icon: 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z' },
  { name: 'Opportunities', href: '/opportunities', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
  { name: 'Activities', href: '/activities', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
...
export default function Sidebar() {
  ...
  // Fetch custom objects for navigation
  const { data: customObjectsData } = useQuery({
    queryKey: ['custom-objects'],
    queryFn: () => api.get('/admin/setup/objects'),
    enabled: !!user,
  });

  const customObjects = customObjectsData?.data?.filter(obj => obj.active !== false) || [];

  // Build custom object nav items
  const customNavigation = customObjects.map(obj => ({
    name: obj.pluralLabel || obj.label,
    href: `/objects/${obj.name}`,
    icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
  }));

  // Combine standard + custom navigation
  const navigation = [...standardNavigation, ...customNavigation];

  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-slate-800 text-white transition-all duration-300 ${
    ...
    >
      ...
      {/* Admin link */}
      {user?.role === 'admin' && (
        <div className="mt-4 px-2 border-t border-slate-700 pt-4">
